...
- 第九次 RAM 启动确认 LuCI 在拨号前/中/后准确显示 No/Yes/No，ubus、procd、进程和
  netdev 的清理状态一致。该次单点公网探测发生在接口上线约 13 秒时并失败，而前一次
  同配置成功；测试脚本改为地址出现后最多轮询公网 60 秒并记录首次可达耗时，以区分
  蜂窝承载建立时序与持续性故障。
- 第十次 RAM 启动持续 60 秒仍未通过单一 `1.1.1.1` ICMP，但 raw-IP、PDP 地址、
  路由、RX/TX 和收尾均正常。为排除单点 ICMP 策略并补齐诊断盲区，后续测试轮询三个
  IPv4 目标和一个 IPv6 目标，记录解析器、非身份类注册/PDP AT 信息，并在进程退出后
  再读取已刷新完成的 `quectel-CM-M` 日志。
